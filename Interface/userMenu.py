...
class userInterface:
    """ This class contains the methods and attributes of first interface that user gets access to after login"""
    def __init__(self,user):
        ...
        self.rent_image=ImageTk.PhotoImage(Image.open("D:/Second Semester/Algorithm/python/Online/finalAssignment/images/rental.png"
        ).resize((60,65)))
        self.rent_label=Label(self.frame1,image=self.rent_image,background="#3198F5")
        self.rent_label.grid(row=0,column=0)

        self.btn1 = Button(self.frame1, text='Rent A Car', font=('Calibri', 16,"bold"), fg="white",background='#3198F5',width=20,
         bd=1,command=self.onRentClick)
        self.btn1.grid(row=1,column=0,pady=10)

        self.park_image=ImageTk.PhotoImage(Image.open("D:/Second Semester/Algorithm/python/Online/finalAssignment/images/parking2.png"
        ).resize((60,65)))
        ...
        self.cancel_image=ImageTk.PhotoImage(Image.open("D:/Second Semester/Algorithm/python/Online/finalAssignment/images/history.png").resize((60,65)))
...
class Transaction:
    """This class shows the transaction history of the user """
    def __init__(self, user):
        """This is an intialization method to add attributes"""
        self.wn=tk.ThemedTk()
        # self.wn=Tk()
        self.wn.configure(bg="#6262ED")
        self.wn.title("User History")
